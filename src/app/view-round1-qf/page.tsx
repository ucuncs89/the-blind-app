  ...
  const getRelatedNodeIds = useCallback(
    (nodeId: string): string[] => {
      ...
      if (nodeId.startsWith("round_2_person_")) {
        ...
        const battle = quarterFinalBattles.find((b) =>
          b.groupWinners.includes(personNumber),
        );
        if (battle) {
      ...
      if (nodeId.startsWith("round_2_wildcard_")) {
        const wildcardNumber = parseInt(
          nodeId.replace("round_2_wildcard_", ""),
        );
        const battle = quarterFinalBattles.find((b) =>
          b.wildcards.includes(wildcardNumber),
        );
        if (battle) {
          relatedIds.push(`quarter_final_${battle.id}`);
        }
      ...
      if (nodeId.startsWith("round_1_group_")) {
        ...
        if (match) {
          ...
          // Add related Quarter Final node
          const battle = quarterFinalBattles.find((b) =>
            b.groupWinners.includes(groupIndex),
          );
          if (battle) {
            relatedIds.push(`quarter_final_${battle.id}`);
          }
