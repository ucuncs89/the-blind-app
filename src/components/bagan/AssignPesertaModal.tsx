...
import { getNodeAssignments, createNodeAssignment, deleteNodeAssignment } from "@/lib/nodeAssignmentAPI";
...
export const AssignPesertaModal = ({ open, onOpenChange, nodeId, nodeName, baganId, currentPesertaId }: AssignPesertaModalProps): React.ReactElement => {
    ...
    // Get all peserta from API
    const { peserta: pesertaList } = usePesertaDB();
    ...
    useEffect(() => {
        if (open) {
            const fetchAssignments = async (): Promise<void> => {
                try {
                    const assignments = await getNodeAssignments(baganId);
    ...
    const assignedPesertaIds = new Set(existingAssignments.filter((a) => a.id !== nodeId).map((a) => a.pesertaId));

    // Filter available peserta (not assigned to other nodes)
    const availablePeserta = pesertaList.filter((p) => !assignedPesertaIds.has(p.id) || p.id === currentPesertaId);
    ...
    const handleAssign = async (): Promise<void> => {
        ...
        try {
            ...
            await createNodeAssignment(assignment);
            // Refresh assignments
            const assignments = await getNodeAssignments(baganId);
            setExistingAssignments(assignments);
            onOpenChange(false);
            ...
            console.error("Failed to assign peserta:", error);
        } finally {
            ...
            <DialogContent className="max-w-md">
                ...
                <div className="py-4">
                    ...
                    {availablePeserta.length === 0 ? (
                        <div className="rounded-lg border border-dashed p-4 text-center text-sm text-muted-foreground">Tidak ada peserta tersedia</div>
                    ) : (
                            ...
                            {availablePeserta.map((peserta) => (
                                <button
                                    ...
                                    className={cn(
                                        ...
                                        selectedPesertaId === peserta.id ? "border-violet-500 bg-violet-500/10 ring-1 ring-violet-500" : "border-border"
                                    )}
                                >
                                    ...
                                    {selectedPesertaId === peserta.id && <div className="h-2 w-2 rounded-full bg-violet-500" />}
                                </button>
                            ))}
                        </div>
                    )}
                ...
                <DialogFooter className="gap-2">
                    {currentPesertaId && (
                        <Button type="button" variant="outline" onClick={handleClear} disabled={isSubmitting} className="text-destructive hover:bg-destructive/10">
                            Hapus Assignment
                        </Button>
                    )}
