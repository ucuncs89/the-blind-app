...
const getSemiFinalY = (battleIndex: number): number => {
  ...
  const qfYPositions = battle.quarterFinals.map((qf) =>
    getQuarterFinalY(qf - 1),
  );
...
export const useBagan1Data = (): UseBagan1DataResult => {
  ...
  const { nodes, edges } = useMemo(() => {
    ...
    const pesertaByGroup = GROUP_LABELS.reduce<Record<string, Peserta[]>>(
      (acc, group) => {
    ...
    const round2Nodes: Node[] = GROUP_LABELS.map((group, groupIndex) => {
      ...
      const assignedPeserta = assignment
        ? pesertaMap.get(assignment.pesertaId)
        : null;
    ...
    const wildcardNodes: Node[] = wildcardConfigs.map((config) => {
      ...
      return {
        ...
        targetPosition: Position.Left,
        data: {
          name: assignedPeserta
            ? assignedPeserta.name
            : `Wildcard ${config.id}`,
    ...
    const quarterFinalNodes: Node[] = QUARTER_FINAL_BATTLES.map(
      (battle, index) => {
        ...
        const groupLabels = battle.groupWinners
          .map((gw) => GROUP_LABELS[gw - 1])
          .join(", ");
    ...
    const championPeserta = championAssignment
      ? pesertaMap.get(championAssignment.pesertaId)
      : null;
    ...
    const thirdPlaceContestantNodes: Node[] = FINAL_BATTLES.map(
      (battle, index) => {
    ...
    const thirdPlacePeserta = thirdPlaceAssignment
      ? pesertaMap.get(thirdPlaceAssignment.pesertaId)
      : null;
    ...
    const round1ToRound2Edges: Edge[] = GROUP_LABELS.flatMap(
      (group, groupIndex) => {
        const groupPeserta = pesertaByGroup[group] || [];
    ...
    const round2ToQuarterFinalEdges: Edge[] = QUARTER_FINAL_BATTLES.flatMap(
      (battle) => {
    ...
    return {
      nodes: [
        ...
        ...finalNodes,
        championNode,
        ...thirdPlaceContestantNodes,
        thirdPlaceNode,
      ],
      edges: [
        ...round1ToRound2Edges,
        ...round2ToQuarterFinalEdges,
        ...qfToSfEdges,
        ...sfToFinalEdges,
        ...finalToChampionEdges,
        ...thirdPlaceContestantEdges,
      ],
    };
