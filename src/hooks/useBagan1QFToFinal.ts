...
const THIRD_PLACE_MATCH_X = 600;
const THIRD_PLACE_WINNER_X = 900;
...
const getThirdPlaceY = (): number => {
    return getChampionY() + PERSON_GAP * 5;
};
...
export const useBagan1QFToFinal = (): UseBagan1QFToFinalResult => {
    ...
    const { nodes, edges } = useMemo(() => {
        ...
        const filteredNodes = allNodes.filter(
            (node) => node.id.startsWith("quarter_final_") || node.id.startsWith("semi_final_") || node.id.startsWith("final_") || node.id === "champion" || node.id.startsWith("third_place")
        );

        // Filter edges: QF → SF, SF → Final, Final → Champion, 3rd Place edges
        const filteredEdges = allEdges.filter(
            (edge) =>
                (edge.source.startsWith("quarter_final_") && edge.target.startsWith("semi_final_")) ||
                (edge.source.startsWith("semi_final_") && edge.target.startsWith("final_")) ||
                (edge.source.startsWith("final_") && edge.target === "champion") ||
                (edge.source.startsWith("third_place_contestant_") && edge.target === "third_place")
        );
        ...
        const repositionedNodes = filteredNodes.map((node) => {
            ...
            // Semi Final nodes
                ...
                const battleIndex = SEMI_FINAL_BATTLES.findIndex((b) => b.id === sfNumber);
                if (battleIndex !== -1) {
                    newNode.position = {
                        x: SEMI_FINAL_X,
            ...
            // Final nodes
                ...
                const battleIndex = FINAL_BATTLES.findIndex((b) => b.id === finalNumber);
                if (battleIndex !== -1) {
                    newNode.position = {
                        x: FINAL_X,
            ...
            // 3rd Place nodes
                ...
                if (node.id === "third_place") {
                    newNode.position = {
                        x: THIRD_PLACE_WINNER_X,
                        y: getThirdPlaceY(),
                    };
                } else if (node.id.startsWith("third_place_contestant_")) {
                    const contestantNumber = parseInt(node.id.replace("third_place_contestant_", ""));
                    const yOffset = contestantNumber === 1 ? -PERSON_GAP / 2 : PERSON_GAP / 2;
                    newNode.position = {
